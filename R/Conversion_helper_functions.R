...
convert_to_fathom <- function(data,
                              region_filter  = NULL,
                              species_filter = NULL,
                              project_filter = NULL,
                              date_start     = NULL,
                              date_end       = NULL) {
  # figure out which project cols exist
  proj_cols <- intersect(names(data), c("Project", "Subproject"))
  if (!is.null(project_filter) && length(proj_cols) == 0) {
    stop("Project filter requested, but neither 'Project' nor 'Subproject' columns exist in `data`.")
  }
  ...
  data |>
    ...
    # Optional Region, Species, Project, and Date filters
    dplyr::filter(
    ...
    # time fields (capture/release) -> build in Brisbane, convert to UTC
    dplyr::mutate(
